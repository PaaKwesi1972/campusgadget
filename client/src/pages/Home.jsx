...
import {
  ...
  MapPin, ChevronRight, Bookmark, Settings, HelpCircle, LogOut,
  Menu, Sparkles,
} from 'lucide-react';
...
export default function Home() {
  ...
  const SidebarContent = () => (
    <div className="flex h-full flex-col px-5 py-6">
      ...
      <div className="space-y-1">{navItems.map(({ key, label, icon: Icon, path }) => <button key={key} onClick={() => { navigate(path); setSidebarOpen(false); }} className={'flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-[12px] font-semibold transition ' + (key === 'home' ? 'bg-[#10143f] text-[#d7a23a] shadow-[0_8px_18px_rgba(16,20,63,0.12)]' : 'text-[#77736c] hover:bg-[#f0ede7] hover:text-[#10143f]')}><Icon className="h-[17px] w-[17px]" strokeWidth={2.1} /><span>{label}</span>{key === 'messages' && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-[#e36b52]" />}</button>)}</div>
  ...
  return (
    <div className="min-h-screen bg-[#fbfaf7] pb-24 font-body text-[#10143f] lg:pb-8">
